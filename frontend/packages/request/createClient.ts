...
/**
 * Create an Axios HTTP client preconfigured with token attachment, automatic refresh, and request queuing.
 *
 * The returned client automatically attaches a stored access token as a `Bearer` Authorization header,
 * triggers `refreshApi` when a 401 response occurs, stores new tokens via `storage`, and queues requests
 * while a token refresh is in progress. It also supports optional request/response interceptors, an
 * optional error handler, configurable timeout, and returns `response.data` by default when `returnDataOnly`
 * is true. Request/response logging is enabled in development mode.
 *
 * @param options - Configuration for the client. Key properties:
 *   - baseURL: base URL for requests.
 *   - storage: token storage implementing getAccessToken/getRefreshToken/setAccessToken/setRefreshToken/clearTokens.
 *   - refreshApi: function to exchange a refresh token for new tokens.
 *   - timeout: request timeout in milliseconds (default 15000).
 *   - requestInterceptor: optional hook to modify requests before they are sent.
 *   - responseInterceptor: optional hooks `onFulfilled`/`onRejected` to customize response handling.
 *   - returnDataOnly: when true (default), the client returns `response.data` instead of the full response.
 *   - errorHandler: optional unified error processing hook.
 * @returns An AxiosInstance configured with automatic Bearer token injection, 401-triggered token refresh with queueing,
 *          optional interceptors, request/response logging in development, and normalized error objects on failures.
 */
